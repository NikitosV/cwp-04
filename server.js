    ...
    client.on('data', (data) => {
        ...
        if (clients[client.id] === reqRemote && data !== reqRemote) {
            ...
            if (remoting[0] === reqRemCopy) {
                ...
                //     automatically into flowing mode and push all of its data to the attached Writable.
            ...
            if (remoting[0] === reqRemEncode) {
                let cryptoStream = crypto.createCipher(algorithm, remoting[3]);  //crypto.createCipher(algorithm, password[, options])
